 ...
 */
	await ImagePicker.launchCamera(options, async (response) => {
		...
		await firebase.database().ref(`register/commerce/motoboyPartner/${this.props.user.id}`).update({
			...
			.then(() => {
				Alert.alert('Atenção', 'Foto de perfil salva com sucesso')
	...
	render(){
		return (
			...
			<HeaderView
				title={'UPLOAD FOTO PERFIL' }
				onBack={() => Alert.alert('Atenção', 'Por favor faça o upload de sua foto de perfil antes de voltar')}
			/>
			{this.state.isLoading ? (
				<Spinner />
			) : (
				<View style={{ flex: 0, alignItems: 'center' }}> 
						<Label style={styles.logoText}>UPLOAD DA FOTO DE PERFIL</Label>
