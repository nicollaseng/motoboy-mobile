						...
						{this.state.loading ? <Spinner /> : (
							<Fragment>
								<TouchableOpacity onPress={this.handleAcceptRide} style={{ alignItems: 'center', justifyContent: 'center'}}>
									...
									/> */}
									<Text style={{ fontSize: 50, color: '#666', marginBottom: 0, marginTop: 20, fontWeigth: '400'}}>{this.state.time}</Text>
								</TouchableOpacity>
		...
		await firebase.database().ref(`register/commerce/motoboyPartner`).once('value', async snap => {
			if(snap.val() !== null){
				...
				if(motoboyActive &&  motoboyActive.length > 0){
					...
					if(motoboySelected.id !== this.props.user.id){
						...
						await firebase.database().ref(`rides/${ride.id}`).update({
							...
							.then(async () => {
								await firebase.database().ref(`register/commerce/list/${ride.restaurant.id}`).once('value',async snapshot => {
									if(snapshot.val() !== null){
										let restaurant = snapshot.val()
										await firebase.database().ref(`register/commerce/list/${ride.restaurant.id}`).update({
											credit: restaurant.credit + ride.taxRestaurant,
											debit: restaurant.debit - ride.taxRestaurant
										})
									}
								})
							})
